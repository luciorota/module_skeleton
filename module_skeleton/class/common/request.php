 ...
 */
define('MODULE_SKELETON_REQUEST_NOTRIM', 1);
define('MODULE_SKELETON_REQUEST_ALLOWRAW', 2);
    ...
    static function _cleanVar($var, $mask = 0, $type = null)
    {
        ...
        if ($mask & 2) {
            ...
            if (is_null($safeHtmlFilter)) {
                $safeHtmlFilter = module_skeleton::getInstance(null, null, 1, 1);
            }
            $var = $safeHtmlFilter->clean($var, $type);
        } else {
            // Since no allow flags were set, we will apply the most strict filter to the variable
            if (is_null($noHtmlFilter)) {
                $noHtmlFilter = module_skeleton::getInstance( /* $tags, $attr, $tag_method, $attr_method, $xss_auto */);
            }
...
/**
 * module_skeletonInput is a class for filtering input from any data source
...
class module_skeleton
{
     ...
     *      <pre>  $filter = & module_skeleton::getInstance();</pre>
     *
     * @static
     *
     * @param array $tagsArray  list of user-defined tags
     ...
     * @param int   $xssAuto    Only auto clean essentials = 0, Allow clean blacklisted tags/attr = 1
     *
     * @return object The module_skeleton object.
     * @since   1.5
     */
    public static function & getInstance($tagsArray = array(), $attrArray = array(), $tagsMethod = 0, $attrMethod = 0, $xssAuto = 1)
    {
        ...
        if (empty ($instances[$sig])) {
            $instances[$sig] = new module_skeleton($tagsArray, $attrArray, $tagsMethod, $attrMethod, $xssAuto);
        }
    ...
    public function clean($source, $type = 'string')
    {
        ...
        switch (strtoupper($type)) {
            ...
            case 'STRING' :
                ...
                if (isset($this) && is_a($this, 'module_skeleton')) {
                    $filter =& $this;
                } else {
                    $filter = module_skeleton::getInstance();
                }
            ...
            default :
                // Check for static usage and assign $filter the proper variable
                if (is_object($this) && get_class($this) == 'module_skeleton') {
                    $filter =& $this;
                } else {
                    $filter = module_skeleton::getInstance();
                }
    ...
    protected function _cleanAttributes($attrSet)
    {
        ...
        for ($i = 0; $i < count($attrSet); ++$i) {
            ...
            if (module_skeleton::checkAttribute($attrSubSet)) {
                continue;
            }
