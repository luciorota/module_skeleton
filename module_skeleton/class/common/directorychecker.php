...
class DirectoryChecker
{
    ...
    public static function getDirectoryStatus($path, $mode = 0777, $redirectFile = null)
    {
        ...
        if (!@is_dir($path)) {
            ...
            $path_status .= "$path (" . _DC_MODULE_SKELETON_NOTAVAILABLE . ") ";
            ...
            $path_status .= "<button class='submit' onClick='this.form.submit();'>" . _DC_MODULE_SKELETON_CREATETHEDIR . "</button>";
            $path_status .= "</form>";
        } elseif (@is_writable($path)) {
            $path_status = "<img src='$pathIcon16/1.png' >";
            $path_status .= "$path (" . _DC_MODULE_SKELETON_AVAILABLE . ") ";
            $currentMode = (substr(decoct(fileperms($path)), 2));
            if ($currentMode != decoct($mode)) {
                $path_status = "<img src='$pathIcon16/0.png' >";
                $path_status .= $path . sprintf(_DC_MODULE_SKELETON_NOTWRITABLE, decoct($mode), $currentMode);
                $path_status .= "<form action='" . $_SERVER['PHP_SELF'] . "' method='post'>";
                $path_status .= "<input type='hidden' name='op' value='setdirperm'>";
                $path_status .= "<input type='hidden' name='mode' value='$mode'>";
                $path_status .= "<input type='hidden' name='path' value='$path'>";
                $path_status .= "<input type='hidden' name='redirect' value='$redirectFile'>";
                $path_status .= "<button class='submit' onClick='this.form.submit();'>" . _DC_MODULE_SKELETON_SETMPERM . "</button>";
                $path_status .= "</form>";
            }
        } else {
            $currentMode = (substr(decoct(fileperms($path)), 2));
...
switch ($op) {
    case "createdir":
        ...
        $msg = (DirectoryChecker::createDirectory($path)) ? _DC_MODULE_SKELETON_DIRCREATED : _DC_MODULE_SKELETON_DIRNOTCREATED;
    ...
    case "setdirperm":
        ...
        $msg = (DirectoryChecker::setDirectoryPermissions($path, $mode)) ? _DC_MODULE_SKELETON_PERMSET : _DC_MODULE_SKELETON_PERMNOTSET;
        redirect_header($redirect, 2, $msg . ': ' . $path);
        exit();
        break;
}
